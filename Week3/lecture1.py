...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov 17 10:50:55 2016
...
def stdDevOfLengths(L):
    ...
    if len(L) == 0:
        return float('NaN')
    lengths_list = [len(string) for string in L]
    mean = sum(lengths_list) / len(lengths_list)
    return ((sum([(length - mean)**2 for length in lengths_list])) / len(lengths_list))**0.5
