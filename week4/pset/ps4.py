...
def r_squared(y, estimated):
    ...
    yArray = np.array(y)
    SEE = ((estimatedArray - yArray)**2).sum()
    mMean = yArray.sum()/float(len(yArray))
    MV = ((mMean - yArray)**2).sum()
    return 1 - SEE / MV
...
def evaluate_models_on_training(x, y, models):
    ...
    for i in range(len(models)):
        ...
        pylab.title('Degree: {} R^2: {}'.format(len(models[i]) - 1, round(error,5)))
        pylab.xlabel('Year')
